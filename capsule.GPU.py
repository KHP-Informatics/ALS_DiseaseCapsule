...
feature_file = '../' +  feature_file # sigSNPs_pca.features.pkl

# read once a time
if prefix.split('_')[1]=='allpca':
    dataset_X=pd.read_pickle(open(feature_file,'rb'))
else:
...
for inputs, labels in dataloaders['validation']:
    ...
    pred_probs.extend(F.softmax(outputs, dim=1).detach().cpu().numpy())
...
def plot_roc(true_labels, pred_probs, prefix):
    ...
    plt.savefig(prefix + '_AUC.png')
    plt.show()

plot_roc(true_labels, pred_probs, 'capsule')
...
# Convert the entire x_test to a torch tensor and move it to the device
in_test = Variable(torch.tensor(x_test).to(device))

# Predict all test samples in one forward pass
y_pred1 = model(in_test).detach().cpu().numpy()
...
print('\t'.join(list(map(str,[acc,precision,recall,f1,auc])))+'\n')
#save results
with open(prefix+'.out.csv','a') as fw:
        fw.write(','.join([model_id,str(seed),prefix]+list(map(str,[acc,precision,recall,f1,auc])))+'\n')
...
print(','.join([prefix]+list(map(str,[acc,precision,recall,f1,auc])))+'\n')
print(y_pred)

...
The software and packages versions used in the manuscript model can be found in `manuscript_environment.yml`.
...
This repository provides the additional scripts used in our ALS implementation, including SNP selection, gene-level annotation, majority imputation of missing genotypes, Gene-PCA feature generation, train/test splitting, CapsNet model training/evaluation, and external-sample inference.
...
10. For external samples, impute missing/absent SNPs from the training data majority genotype, project them into the training-derived Gene-PCA space, and generate predictions using trained CapsNet weights.
...
### 8. Process external samples and generate predictions

The following two scripts support inference on new samples without reprocessing the training and external datasets together. This is the projected Gene-PCA workflow described in the manuscript.

First, build external per-gene genotype matrices. The script uses the SNP order from the training gene matrices and imputes missing genotypes, including SNPs absent from the external VCF, using the most common genotype observed in the training data.

```bash
python scripts/get_genes.py \
  --chrom 1 \
  --vcf external/all.sig.1.vcf \
  --training-genes-dir prepared/chr1/genes \
  --annotation annotations/chr1.variant_function.uniqGene \
  --train-idx splits/balanced/train_val.unique.idx \
  --outdir external_prepared
```

This writes:

```text
external_prepared/chr1/genes/<GENE>.pkl
external_prepared/chr1/samples.txt
external_prepared/chr1/imputation_report.tsv
```

Then project the external samples into the PCA models fitted on the training samples by `03_gene_pca.py`:

```bash
python scripts/pca_separate.py \
  --external-genes-dir external_prepared/chr1/genes \
  --training-pca-dir prepared/chr1/pca \
  --outdir external_prepared/chr1/pca
```

After repeating this for all chromosomes, combine the external Gene-PC features. Labels are optional, so they can be omitted for unlabelled external samples:

```bash
python scripts/04_combine_gene_pcs.py \
  --pca-root external_prepared \
  --output external_sigSNPs_pca.features.pkl
```

Finally, generate predictions with trained CapsNet weights:

```bash
python scripts/06_train_capsnet.py \
  --features external_sigSNPs_pca.features.pkl \
  --weights capsule_pca_1.0model_1.pt \
  --predict-only \
  --predict-all \
  --outdir external_predictions
```

This produces:

```text
external_predictions/predictions.csv
```

The `probability_ALS_case` column is the model probability assigned to the ALS-case class. These predictions are research outputs and should not be used for clinical decision-making.

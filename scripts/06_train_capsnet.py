...
def main():
    ...
    parser.add_argument("--weights", type=Path, default=None, help="Optional trained CapsNet weights to load.")
    parser.add_argument("--predict-only", action="store_true", help="Only run inference using --weights; do not train.")
    parser.add_argument("--predict-all", action="store_true", help="When predicting, ignore train/test indices and predict all rows in --features.")
    ...
    if args.predict_only:
        if args.weights is None:
            raise ValueError("--predict-only requires --weights")
        if args.predict_all:
            predict_idx = np.arange(X.shape[0])
        else:
            if args.test_idx is None:
                raise ValueError("Provide --test-idx or use --predict-all for prediction.")
            predict_idx = read_idx(args.test_idx)
        x_pred = torch.tensor(X[predict_idx], dtype=torch.float32)
        pred_loader = DataLoader(TensorDataset(x_pred), batch_size=args.batch_size, shuffle=False)
        model.eval()
        probs = []
        with torch.no_grad():
        ...
        prediction_df = pd.DataFrame({
            "row_index": predict_idx,
            "probability_ALS_case": probs,
            "predicted_class": pred,
        })
        prediction_df.to_csv(args.outdir / "predictions.csv", index=False)
        print(f"Saved predictions for {len(prediction_df)} samples to {args.outdir / 'predictions.csv'}")
        return

    if y is None:
        raise ValueError("Training/evaluation requires labels in the feature pickle. For external inference use --predict-only --weights.")

    if args.train_idx is None or args.test_idx is None:
        raise ValueError("Training/evaluation requires --train-idx and --test-idx.")

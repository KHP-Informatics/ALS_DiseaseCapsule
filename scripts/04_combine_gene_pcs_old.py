#!/usr/bin/env python3
"""Combine all Gene-PCA embeddings into one feature matrix for model training."""
...
def main():
    ...
    parser.add_argument("--labels", required=True, type=Path, help="Labels file with Pheno column, same sample order as features.")
    ...
    with args.output.open("wb") as out:
        pickle.dump((X, y), out)
    print(f"Saved {X.shape[0]} samples x {X.shape[1]} Gene-PCs to {args.output}")

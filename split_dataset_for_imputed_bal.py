...
# Extract the cases and controls from the test set based on the indices
test_set_labels = labels_df.loc[test_set_indices,'Pheno']

cases_indices = [index for index, label in zip(test_set_indices, test_set_labels) if label == 2]
controls_indices = [index for index, label in zip(test_set_indices, test_set_labels) if label == 1]

# Step 4: Randomly sample an equal number of cases and controls for the new balanced test set
min_class_size = min(len(cases_indices), len(controls_indices))
balanced_size = min_class_size // 2  # Taking 50% of the smaller class

balanced_cases_indices = np.random.choice(cases_indices, balanced_size, replace=False)
balanced_controls_indices = np.random.choice(controls_indices, balanced_size, replace=False)
len(balanced_cases_indices)
te_idx = np.concatenate((balanced_cases_indices, balanced_controls_indices))
len(te_idx)
